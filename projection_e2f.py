...
def main():
    ...
    # P_flir_part = P_flir[:,1:2]; #前两列 [p1,p2]
    # P_flir_const = -Z*P_flir[:,3]-P_flir[:,4]; # 
    ...
    # 投影到事件相机坐标系
    print("投影到事件相机坐标系...")
    event_proj = P_event @ world_coords
    event_proj = event_proj / event_proj[2, :]  # 齐次坐标归一化
    
    # 获取映射坐标
    map_x = event_proj[0, :].reshape(flir_h, flir_w)
